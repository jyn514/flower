...
4. For each file, run `flower transform` on all transformers. Save the output to `public`. Transformers are run in an order determined by `build.clj`.
...
1. `render`: Expand the template language (see [sunflower](language.md)).
